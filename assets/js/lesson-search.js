...
(function () {
  ...
  function init() {
    ...
    function flashElement(el, behavior) {
      ...
      setTimeout(function () { el.classList.remove("ls-flash"); }, 1600);
    }

    function jumpTo(entry) {
      close();
      flashElement(entry.el);
    }
    ...
    if (location.hash.length > 1) {
      ...
      if (hashTarget && (hashTarget.classList.contains("scene") || hashTarget.classList.contains("end"))) {
        setTimeout(function () { flashElement(hashTarget, "auto"); }, 60);
      }
    }
  }
