...
- `learning-graph/` — Section 8 (the Claude Agent SDK) rebuilt as a knowledge graph: 44 micro-topics, 64 prerequisite edges, 48 encompassing edges, 60 knowledge points and 164 questions, with a browser app that schedules what you should do next. It implements the mechanics from *The Math Academy Way* — knowledge frontier, scaffolded mastery learning, targeted remediation, fractional implicit repetition, review compression, spaced repetition, and an adaptive placement diagnostic — over data formatted after `withmarbleapp/os-taxonomy`. Only Section 8 gets this treatment because it is the heaviest domain on the exam and the one with no official course. Open `learning-graph/index.html`; progress is stored under `ccaf_learning_graph_v1`. Source data is in `learning-graph/data/*.json`, validated with `node learning-graph/scripts/validate.mjs`.
